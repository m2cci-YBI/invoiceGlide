...
export class LandingComponent implements OnInit {
  BRAND = 'InvoiceGlide';
  DOMAIN = 'invoiceglide.com';
  currentYear = new Date().getFullYear();
  plans: Plan[] = [];
  @ViewChild('tiltCard', { static: false }) tiltCard?: ElementRef<HTMLDivElement>;
  @ViewChild('tiltScene', { static: false }) tiltScene?: ElementRef<HTMLDivElement>;
  private defaultTilt = 'rotateY(-10deg) rotateX(2deg)';
  private maxRotate = 10; // degrees
  private raf?: number;
  ...
  onTiltMove(ev: MouseEvent) {
    if (!this.tiltCard || !this.tiltScene) return;
  ...
  private scheduleTransform(t: string, elevateShadow: boolean) {
  ...
  priceLabel(plan: Plan): string {
    if (!plan.priceCents || plan.priceCents === 0) return '$0';
