...
export interface CompanySettings {
  ...
  templateTaxes: { name: string; ratePct: number }[];
  templateDiscounts: { name: string; type: 'percent' | 'amount'; value: number }[];
  invoiceStyles: { [key: string]: TextStyle };
...
export interface TextStyle {
  ...
  fontWeight?: string; // normal | 500 | 600 | 700
}
...
<div style="margin:0;padding:0;background-color:#ECFDF5;">
  ...
  loadFromBackend(): Observable<CompanySettings> {
    return this.http.get<CompanySettings>(`${INVOICE_API}/settings`).pipe(
      tap(remote => {
        if (remote) {
          const defaults = this.settings;
          const mergedStyles = { ...(defaults.invoiceStyles || {}), ...((remote as any).invoiceStyles || {}) };
          this.settings = {
            ...defaults,
            ...remote,
            templateTaxes: Array.isArray((remote as any).templateTaxes) ? (remote as any).templateTaxes : defaults.templateTaxes,
            templateDiscounts: Array.isArray((remote as any).templateDiscounts) ? (remote as any).templateDiscounts : defaults.templateDiscounts,
            invoiceStyles: mergedStyles,
          } as CompanySettings;
          // Cache locally too
          localStorage.setItem(STORAGE_KEY, JSON.stringify(this.settings));
  ...
  formatCurrency(n: number, code?: string) {
