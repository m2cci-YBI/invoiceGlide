...
export class NewInvoiceComponent implements OnInit {
  ...
  invoiceTaxes: { name: string; ratePct: number }[] = [
    { name: 'HST', ratePct: 13 },
  ];
  subtotal = 0;
  tax = 0;
  ...
  invoiceDiscounts: { name: string; type: 'percent' | 'amount'; value: number }[] = [];
  perDiscountTotals: { name: string; amount: number }[] = [];
  discountTotal: number = 0;
  private draftKey = 'draft_invoice';
  lastDeleted: { idx: number; line: any } | null = null;
  showPreview = false;
  // Cached preview data to avoid expensive recomputation on every CD tick
  previewInvoiceData: any = null;

  customers: ClientDto[] = [];
  filteredCustomers: ClientDto[] = [];
  ...
  ngOnInit(): void {
    this.clientsApi.list().subscribe(clis => {
      ...
      try {
        ...
        if (saved) {
          ...
          if (selId) this.selectedClient = this.customers.find(c => c.id === selId) || null;
        }
      } catch {}
    });
    ...
    if (saved) {
      try {
        ...
        if (parsed.invoiceTaxes) this.invoiceTaxes = parsed.invoiceTaxes;
        if (parsed.issueDate) this.issueDate = parsed.issueDate;
        if (parsed.dueDate) this.dueDate = parsed.dueDate;
        if (parsed.invoiceCurrency) this.invoiceCurrency = parsed.invoiceCurrency;
        if (Array.isArray(parsed.invoiceDiscounts)) this.invoiceDiscounts = parsed.invoiceDiscounts;
      } catch {}
    }
    if (!this.issueDate) {
  ...
  calculateTotals() {
    ...
    for (const l of this.lines) {
      const lineNet = round2((Number((l as any).qty) || 0) * (Number((l as any).price) || 0));
  ...
  updateLine(index: number, patch: any) {
  ...
  updatePreviewInvoice() {
    ...
    this.previewInvoiceData = {
      ...
      lines: this.lines.map((l: any) => ({ description: l.description, qty: l.qty, unit: l.unit, price: l.price })),
      invoiceTaxes: this.invoiceTaxes,
      invoiceDiscounts: this.invoiceDiscounts,
  ...
  removeInvoiceDiscount(idx: number) {
  ...
  saveInvoice() {
    ...
    const body = {
      clientId: (this.selectedClient as ClientDto).id,
      issueDate: this.issueDate,
      dueDate: this.dueDate || this.issueDate,
      currency: this.invoiceCurrency,
      lines: this.lines.map((l: any) => ({ description: l.description, qty: Number(l.qty) || 0, unit: l.unit || '', unitPrice: Number(l.price) || 0 })),
    ...
    this.invoicesApi.create(body).subscribe({
      next: () => { try { localStorage.removeItem(this.draftKey); } catch {} this.toast.success('Invoice issued'); this.router.navigate(['/dashboard']); },
  ...
  downloadInvoice() {
    ...
    const linesHtml = this.lines.map((l: any) => `
      <tr>
    ...
    const discountsHtml = this.perDiscountTotals.map((d, di) => {
      ...
      const label = src && src.type === 'percent'
        ...
        : `${this.escapeHtml(d.name)} (${this.settingsService.formatCurrency(Number((src as any)?.value) || 0, currency)})`;
      return `
